...
contract LRTConfigUpdateAssetStrategyTest is LRTConfigTest {
    ...
    function test_RevertWhenStrategyHasFundsInNDC() external {
        address tstrategy = address(new MockStrategy(address(stETH), 1 ether));
        vm.startPrank(admin);
        lrtConfig.updateAssetStrategy(address(stETH), tstrategy);
    ...
    function test_UpdateAssetStrategy() external {
        address tstrategy = address(new MockStrategy(address(stETH), 0));
